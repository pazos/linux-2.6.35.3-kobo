...
static inline int get_idx_vwdev_by_wlphy(struct wl_priv *wl, struct wiphy *wiphy)
{
	int i = 0;
	for (i = 0; i < VWDEV_CNT; i++) {
		if ((wl->vwdev[i] != NULL) && (wl->vwdev[i]->wiphy == wiphy))
				return i;
	}
	return -1;
}

static inline struct wiphy * get_wiphy_by_netdev(struct wl_priv *wl,
	struct net_device *ndev)
{
	int i = 0;
	for (i = 0; i < VWDEV_CNT; i++) {
		if ((wl->vwdev[i] != NULL) && (wl->vwdev[i]->netdev == ndev))
			return wl->vwdev[i]->wiphy;
	}

	return wl_to_wiphy(wl);

}
#define free_vwdev_by_index(wl, __i) do { wl->vwdev[__i] = NULL; } while (0)
