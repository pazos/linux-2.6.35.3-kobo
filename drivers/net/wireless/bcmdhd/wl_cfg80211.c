                              ...
                              struct vif_params *params)
{
	...
	if (timeout > 0 && test_bit(WLP2P_STATUS_IF_ADD, &wl->p2p_status)) {
		...
		vwdev = wl_alloc_wdev(sizeof(struct wl_iface),
		                     &wl_cfg80211_get_sdio_func()->dev);
		if (unlikely(IS_ERR(vwdev)))
			return ERR_PTR(-ENOMEM);
		WL_INFO((" virtual interface(%s) is vwdev created \n", wl->p2p_vir_ifname));
...
wl_cfg80211_del_virtual_iface(struct wiphy *wiphy, struct net_device *dev)
{
	...
	index = get_idx_vwdev_by_wlphy(wl, wiphy);
	if (index >= 0)
                                ...
                                struct vif_params *params)
{
	...
	WL_DBG(("p2p_vif_created (%d) p2p_on (%d)\n", wl->p2p_vif_created, wl->p2p_on));

	if (ap && wl->p2p_vif_created) {
		wl_dongle_save_eventmsg(wl);
		wl_dongle_disable_eventmsg(wl);
		/* Temporary use channel 11, in case GO will be changed with set_channel API  */
		chspec = wf_chspec_aton(WL_P2P_TEMP_CHAN);
		/* For P2P mode, use P2P-specific driver features to create the
		 * bss: "wl p2p_ifadd"
	...
	struct beacon_parameters *info)
{
	...
	s32 ie_offset = 0;
	s32 bssidx = wl_to_p2p_bss_bssidx(wl, P2PAPI_BSSCFG_CONNECTION);

	s32 auth = 0;
	s32 infra = 1;
	s32 wpa_auth = WPA2_AUTH_PSK;
	u8 wsec = AES_ENABLED;
	...
	if (!is_bssup) {
		/* TODO : There is no way to identify security setting for AP
		*   So we have to parse WPA_IE and RSN_IE and set security refering to IE.
		*/
	...
	const wl_event_msg_t *e, void *data)
{
	...
	if (wl->conf->mode == WL_MODE_AP) {
		...
		printk("ERROR @wl_notify_connect_status : event %d ",  ntoh32(e->event_type));
		if (wl_is_linkup(wl, e)) {
			...
			printk("wl->status 0x%x ", (int)wl->status);
			if (test_bit(WL_STATUS_CONNECTED, &wl->status)) {
				printk("cfg80211_disconnected");
...
static s32 wl_update_bss_info(struct wl_priv *wl, struct net_device *ndev)
{
	...
	s32 err = 0;
	struct wiphy *wiphy;
	wiphy = get_wiphy_by_netdev(wl, ndev);
...
static s32 wl_dongle_restore_eventmsg(struct wl_priv *wl)
{
	...
	/* Setup event_msgs */
	bcm_mkiovar("event_msgs", wl->last_eventmask, WL_EVENTING_MASK_LEN, iovbuf,
		sizeof(iovbuf));
	err = wl_dev_ioctl(ndev, WLC_SET_VAR, iovbuf, sizeof(iovbuf));
	if (unlikely(err)) {
		WL_ERR(("Get event_msgs error (%d)\n", err));
